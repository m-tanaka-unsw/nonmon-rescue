...
## Alternative components of Prescue [not used] 
##  - these further break up the probability: 
##    P(new only), P(old only), P(old and new) 
prob.piece.kummer <- function(N,sb,delta,mu, sd){
   require("gsl")
   pfix <- pfix.exact(sb-delta)
   pnew <- pold <- poldnew <- c() 
   for (i in 1:length(N)) {
      alpha <- 2*N[i]*mu  
      gamma <- 2*N[i]*sd
      Bconst <-   exp(-pfix*N[i]*mu/delta)
      A <- pfix*N[i]
      B <- pfix*N[i]*mu/delta
      f.Ag <- hyperg_1F1(alpha,2*alpha,-A-gamma)
      f.Bg <- hyperg_1F1(alpha,2*alpha, B-gamma)
      f.BAg <- hyperg_1F1(alpha,2*alpha, B-A-gamma)

      f1.bot <- hyperg_1F1(alpha,2*alpha,-gamma)
      pnew[i] <- (f.Ag - f.BAg*Bconst)/f1.bot
      pold[i] <- (f.Bg - f.BAg)*Bconst/f1.bot
      poldnew[i] <- 1-(f.Ag + f.Bg*Bconst- f.BAg*Bconst)/f1.bot
   }
   list(pnew=pnew, pold=pold, poldnew=poldnew)
}
...
onesim <- function(p){
   ...
   with(p, {
      ...
      tcap <- tmax*10     # run very long time unless extinction or escape
      wt <- c(N0-resc0)   # initial number of wildtype
      muts <- c(rbinom(1,wt[1],mu))  # first gen mutants 
      for (i in 2:tcap) {  # subsequent generations 
         wt[i] <- rpois(1, wt[i-1]*(1-delta))  # randomly draw next gen
         muts[i] <- rbinom(1,wt[i],mu)         # rescue mutants 
         wt[i] <- wt[i]-muts[i]                # conservative mutation 
         lambda <-  (1+sb-delta)*resc[i-1]     # resc allele fitness 
         resc[i] <- rpois(1, lambda) + muts[i] # next gen rescue carriers
...
plot.components <- function(lo=1, hi=5){
   ...
   ## ----- OLD VERSION with mutually exclusive pieces: -----
   if (FALSE) {  # turned off for now
#   if (TRUE) {
   ## Use the kummer version of the function 
      plot(1,1, pty="n", xaxt="n", log="x", xlim=c(10^lo,10^hi), ylim=c(0,1)
         , xlab="Population size N", ylab="Probability of rescue")
      eaxis(1) 
      ppk <- prob.piece.kummer(N_, p$sb,p$delta,p$mu,p$sd)
      ## Plot the total rescue probability
      lines(N_, ppk$pold+ppk$pnew+ppk$poldnew, col= cola[1], lty=1)
      ## Same but compute total directly: (just to check) 
      lines(N_, prob.TW(N_, p$sb,p$delta,p$mu,p$sd), col="forestgreen")
      lines(N_, ppk$pold, col= cola[2] , lty=2) 
      lines(N_, ppk$pnew, col= cola[3]   , lty=5) 
      lines(N_, ppk$poldnew, col= cola[4] , lty=4)
      legend("topleft", c("Total","Old only","New only","Old & new")
           , lty=c(1,2,5,4), col=cola
           , bty="n", cex=0.9)
   }
   ## -------------------------------------------------------
   dev.off()
...
get.min.N.old <- function(p, res=10000, say=FALSE, interv=c(10,10^5)){
...
get.min.N <- function(p, say=FALSE, interv=c(10,10^5)){
   res <- 500 # hard code this time 
   ## Round 1
...
## Plot worst N vs mu, and vs sd
plot.worst.N <- function(){
   require("sfsmisc") 
   mu.seq <- 10^seq(-6,-4.2, length.out=80) 
   sd.seq <- 10^seq(-4,-1, length.out=80)

   wN.series <- function(mu.seq,sb){ # worst N over seq of mu, given sb
      worst.N <-  c()
      for (i in 1:length(mu.seq) ) {
         p <- setp(sb=sb, mu=mu.seq[i])
         worst.N[i] <- get.min.N(p) 
      }
      worst.N
   }
   wN.series.sd <- function(sd.seq,sb){ # for sd sequence, given sb
      worst.N <- c()
      for (i in 1:length(sd.seq) ) {
         p <- setp(sd=sd.seq[i], sb=sb)
         worst.N[i] <- get.min.N(p) 
      }
      worst.N
   }
   ...
   wN05 <- wN.series(mu.seq, sb=0.05)
   wN02 <- wN.series(mu.seq, sb=0.02)
   wN011 <- wN.series(mu.seq, sb=0.011)
   plot(mu.seq, wN05, xlim=c(10^-6, 10^-4),ylim=c(0,max(wN05,wN02,wN011))
      , log="x"
      , ylab="Locally worst population size" 
      , xlab=expression("Mutation rate, "*mu), t="n", xaxt="n")
   eaxis(1, n.axp=1, at.small=F) 
   title("A", adj=0)
   lines(mu.seq, wN011, col=2)
   lines(mu.seq, wN02, col=4)
   lines(mu.seq, wN05, col=1)
#   points(mu.seq, wN011, pch=16, col=2)
#   points(mu.seq, wN02, pch=16, col=4)
#   points(mu.seq, wN05, pch=16, col=1)
   legend("topright", c(expression(s[b]*" = 0.011"),
                        expression(s[b]*" = 0.02"),
                        expression(s[b]*" = 0.05")), #pch=16,
          col=c(2,4,1), lty=1, bty="n") 

   wN05 <- wN.series.sd(sd.seq, sb=0.05)
   wN02 <- wN.series.sd(sd.seq, sb=0.02)
   wN011 <- wN.series.sd(sd.seq, sb=0.011)
   plot(sd.seq,wN05, log="x", ylim=c(0, max(wN05,wN02,wN011))
      , ylab="Locally worst population size", xaxt="n"
      , xlab=expression("Deleterious effect, "*s[d]), t="n")
   eaxis(1, n.axp=1, at.small=F) 
   title("B", adj=0)
   lines(sd.seq, wN011, col=2)
   lines(sd.seq, wN02, col=4)
   lines(sd.seq, wN05, col=1)
#   points(sd.seq, wN011, pch=16, col=2)
#   points(sd.seq, wN02, pch=16, col=4)
#   points(sd.seq, wN05, pch=16, col=1)
   legend("topright", c(expression(s[b]*" = 0.011"),
                        expression(s[b]*" = 0.02"),
                        expression(s[b]*" = 0.05")),   #pch=16,
          col=c(2,4,1), lty=1, bty="n") 
