...
setp <- function(sb = 0.05,      # rescue selective effect
   ...
   list(sb=sb, delta=delta, sd=sd, 
        N0=N0, mu=mu,
        tmax=tmax, resc0=resc0, simnum=simnum,
...
plot.wf.multi <- function(chains=c(32,32,32,64,
   ...
   par(mfrow=c(2,4), oma=c(3,3,0.2,0.2), mar=c(2,2,1,1), cex=0.55
     , xpd= TRUE)
   
   for (j in 1:8) { # each parameter set
      p <- choose.pset(j)   # Get parameter values for set j
      mt <- paste("N=",p$N,", sd=",p$sd,", mu=", p$mu, sep="")
   ...
   mtext("Frequency",1, outer=T, cex=0.7, line=1.8)
   mtext("Probability",2, outer=T, cex=0.7, line=1.8)
   dev.off()
}
...
#   sim.orr.wright.over.N(setp(p0.threshold=1,density=16),ext="p1")  # all except p0=1
...
plot.init.rare.allele <- function(){
   require("sfsmisc") 
   pdf("prob-rescue-v-survival.pdf", height=2.6,width=2.6)
   co <- c("black", "purple", "dodgerblue") 
   par(mar=c(4,4,1,1) , cex=0.7)
   fn <- c("dat-orr-wright-sim-default",
           "dat-orr-wright-sim-p1",
           "dat-orr-wright-sim-p0.5")
   plot(1,1, t="n", log="x", cex=0.9, xlim=c(10^2,10^5), ylim=c(0,1.12)
      , las=1, xaxt="n"
      , xlab="Population size N", ylab="Probability of rescue or survival"
      , cex.lab=1.2)   
      eaxis(1, f.smalltcl=0, n.axp=1)     # nice x axis
   for (i in 1:3) {
      d <- read.table(fn[i],header=T)
      points(d$N,d$presc, pch=16, col=co[i] , cex=0.8)      
      lines(d$N,d$presc, col=co[i], lty=1, lwd=0.7)
   }
   legend("topleft", c("all p", "p<1", "p<0.5"),
          lty=1, lwd=0.7, pch=16, col=co, cex=0.8, bty="n")
   dev.off()
}
